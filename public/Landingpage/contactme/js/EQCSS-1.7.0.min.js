...
(function(root, factory) {
    ...
    EQCSS.load = function() {
        ...
        for (i = 0; i < link.length; i++)
            if (link[i].getAttribute("data-eqcss-read") === null &&
                link[i].rel === "stylesheet") {
                if (link[i].href)(function() {
                    var xhr = new XMLHttpRequest;
    ...
    EQCSS.parse = function(code) {
        ...
        code = code.replace(/\/\*[\w\W]*?\*\//g, "");
        code = code.replace(/@element/g, "\n@element");
        code = code.replace(/(@element.*?\{([^}]*?\{[^}]*?\}[^}]*?)*\}).*/g, "$1");
        code.replace(/(@element.*(?!@element))/g,
    ...
    EQCSS.register = function(queries) {
        ...
        if (Object.prototype.toString.call(queries) === "[object Array]") {
            for (var i = 0; i < queries.length; i++) EQCSS.data.push(queries[i]);
            EQCSS.apply()
        }
    };
        ...
        for (i = 0; i < EQCSS.data.length; i++) {
            ...
            for (j = 0; j < elements.length; j++) {
                ...
                test_conditions: for (k = 0; k < EQCSS.data[i].conditions.length; k++) {
                    computed_style = window.getComputedStyle(elements[j],
                        null);
                    ...
                    if (EQCSS.data[i].conditions[k].unit === "vw") {
                        ...
                        null && EQCSS.data[i].conditions[k].unit != "px" && EQCSS.data[i].conditions[k].unit != "%") {
                    ...
                    switch (EQCSS.data[i].conditions[k].measure) {
                        ...
                        case "min-height":
                            ...
                            if (EQCSS.data[i].conditions[k].unit === "%") {
                                ...
                                if (!(parent_height /
                                        element_height <= 100 / final_value)) {
                                    test = false;
                                    break test_conditions
                                }
                            }
                        ...
                        case "min-scroll-x":
                            ...
                            if (!element.hasScrollListener)
                                if (element === document.documentElement || element === document.body) window.addEventListener("scroll", function() {
                                    EQCSS.throttle();
                                    element.hasScrollListener = true
                                });
                                else element.addEventListener("scroll", function() {
                                    EQCSS.throttle();
                                    element.hasScrollListener = true
                                });
                            if (recomputed === true || EQCSS.data[i].conditions[k].unit === "px") {
                                ...
                                else element_size = parseInt(computed_style.getPropertyValue("width"));
                        ...
                        case "min-scroll-y":
                            ...
                            if (recomputed === true || EQCSS.data[i].conditions[k].unit === "px") {
                                ...
                                var element_scroll_size =
                                    elements[j].scrollHeight;
                                var element_size;
                                if (elements[j] === document.documentElement || elements[j] === document.body) element_size = window.innerHeight;
                                else element_size = parseInt(computed_style.getPropertyValue("height"));
                                if (!(element_scroll / (element_scroll_size - element_size) * 100 >= final_value)) {
                                    test = false;
                                    break test_conditions
                                }
                        ...
                        case "max-scroll-x":
                            ...
                            if (!element.hasScrollListener)
                                if (element === document.documentElement || element === document.body) window.addEventListener("scroll",
                                    function() {
                                        EQCSS.throttle();
                                        element.hasScrollListener = true
                                    });
                        ...
                        case "max-scroll-y":
                            ...
                            if (recomputed === true || EQCSS.data[i].conditions[k].unit === "px") {
                                ...
                                if (!(element_scroll / (element_scroll_size - element_size) *
                                        100 <= final_value)) {
                                    test = false;
                                    break test_conditions
                                }
                        ...
                        case "min-characters":
                            ...
                            } else if (!(elements[j].textContent.length >= final_value)) {
                                test = false;
                                break test_conditions
                            }
                            break;
                        ...
                        case "max-children":
                            ...
                            break;
                        case "min-lines":
                            element_height = parseInt(computed_style.getPropertyValue("height")) - parseInt(computed_style.getPropertyValue("border-top-width")) - parseInt(computed_style.getPropertyValue("border-bottom-width")) - parseInt(computed_style.getPropertyValue("padding-top")) - parseInt(computed_style.getPropertyValue("padding-bottom"));
                            element_line_height =
                                computed_style.getPropertyValue("line-height");
                            ...
                            } else element_line_height = parseInt(element_line_height);
                            ...
                            break;
                        case "max-lines":
                            element_height = parseInt(computed_style.getPropertyValue("height")) - parseInt(computed_style.getPropertyValue("border-top-width")) - parseInt(computed_style.getPropertyValue("border-bottom-width")) -
                                parseInt(computed_style.getPropertyValue("padding-top")) - parseInt(computed_style.getPropertyValue("padding-bottom"));
                            ...
                            break;
                        case "orientation":
                            if (EQCSS.data[i].conditions[k].value ===
                                "square")
                ...
                if (test === true) {
                    ...
                    css_code = css_code.replace(/eval\( *((".*?")|('.*?')) *\)/g, function(string, match) {
                        return EQCSS.tryWithEval(elements[j],
                            match)
                    });
                    ...
                    css_code = css_code.replace(/(\d*\.?\d+)(?:\s*)(ew|eh|emin|emax)/gi, function(match, $1, $2) {
                        switch ($2) {
                            case "ew":
                                ...
                                break;
                            case "eh":
                                return elements[j].offsetHeight /
                                    100 * $1 + "px";
                                break;
                            case "emin":
    ...
    EQCSS.reset = function() {
        ...
        for (var j = 0; j < tag.length; j++)
            for (var k = 0; k < tag[j].attributes.length; k++)
                if (tag[j].attributes[k].name.indexOf("data-eqcss-") === 0) tag[j].removeAttribute(tag[j].attributes[k].name)
    };
    EQCSS.domReady = function(fn) {
        ...
        if (doc.readyState === "complete") fn.call(window,
            "lazy");
        else {
    ...
    EQCSS.throttle = function() {
        ...
        } else EQCSS_throttle_queued = true
    };
    ...
    window.addEventListener("mousedown", function(e) {
        if (e.which === 1) EQCSS_mouse_down = true
    });
    window.addEventListener("mousemove", function() {
        if (EQCSS_mouse_down) EQCSS.throttle()
    });
